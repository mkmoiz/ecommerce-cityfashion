...
import { adminGet, adminPut, uploadProductImage, getAdminToken } from "@/utils/adminApi";
import { toast } from "react-hot-toast";

export default function EditHeroSlide({ params: paramsPromise }) {
    ...
    useEffect(() => {
        if (hasToken) {
            adminGet(`/admin/hero/${params.id}`).then(data => {
                setForm({
                    title: data.title || "",
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await adminPut(`/admin/hero/${params.id}`, form);
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded shadow">
                ...
                <button type="submit" className="bg-black text-white px-4 py-2 rounded">Update Slide</button>
            </form>
        </div>
    )
}
